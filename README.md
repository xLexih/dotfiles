...
Text-based configs (`.conf`, `.ini`, `.lua`, `.sh`) are **concatenated** across all layers that exist, producing a merged result. Structured formats (`.json`, `.jsonc`, `.qml`, `.svg`, `.toml`, `.yaml`) use **last-wins** — the most specific layer replaces the others entirely.
